...
test('build output exists', async (t) => {
  // Check main file
  const mainExists = fs.existsSync(path.resolve('dist/translate-content.mjs'));
  assert.equal(mainExists, true, 'Main translation file should exist in dist/');

  // Check binary file
  const binExists = fs.existsSync(path.resolve('dist/bin/translate.mjs'));
  assert.equal(binExists, true, 'Binary file should exist in dist/bin/');
});

test('binary file has correct permissions', async (t) => {
  const binPath = path.resolve('dist/bin/translate.mjs');
